...
try {
    ...
    if($techId) {
        ...
        else if($data['status'] === 'Unresolved' && $oldStatus !== 'Unresolved') {
            $stmt = $pdo->prepare("UPDATE technical_staff SET unresolve = unresolve + 1 WHERE technical_id = ?");
            $stmt->execute([$techId]);
        }
        // If moving from unresolved to resolved
        else if($data['status'] === 'Resolved' && $oldStatus === 'Unresolved') {
            $stmt = $pdo->prepare("UPDATE technical_staff SET resolve = resolve + 1, unresolve = unresolve - 1 WHERE technical_id = ?");
            $stmt->execute([$techId]);
        }
        // If moving from resolved to unresolved
        else if($data['status'] === 'Unresolved' && $oldStatus === 'Resolved') {
            $stmt = $pdo->prepare("UPDATE technical_staff SET resolve = resolve - 1, unresolve = unresolve + 1 WHERE technical_id = ?");
            $stmt->execute([$techId]);
        }
    }
